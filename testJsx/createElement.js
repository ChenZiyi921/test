...
function createElement(opts) {
    // opts.elementName, opts.attributes = {}, ...opts.children
    if (opts.attributes === null) {
        opts.attributes = {}
    }
    const tag = HTML_TAGS[opts.elementName]
    ...
    Object.keys(opts.attributes).forEach(prop => {
        if (prop in attrs) {
            el.setAttribute(attrs[prop], opts.attributes[prop])
        }
        if (prop in EVENT_HANDLERS) {
            el.addEventListener(EVENT_HANDLERS[prop], opts.attributes[prop])
        }
    })
    if ('style' in opts.attributes) {
        const styles = opts.attributes.style
    ...
    opts.children.forEach(childNode => {
        // debugger
